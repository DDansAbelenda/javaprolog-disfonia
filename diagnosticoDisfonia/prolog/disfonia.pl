...
diagnostica(Paciente,'Disfonía Funcional', Sintomas, Signos, Laringoscopia):-
      long(Sintomas,Length1),Length1>0, findall(X,sintoma(X),List1), todos_miembros(Sintomas,List1),  % que todos pertenezcan a los registrados
      long(Signos,Length2),Length2>0,   findall(X,signo(X),List2),   todos_miembros(Signos,List2),    % que todos pertenezcan a los registrados
      long(Laringoscopia,1),
